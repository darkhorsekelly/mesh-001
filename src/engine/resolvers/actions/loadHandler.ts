// ===============================================
// LOAD ACTION HANDLER
// ===============================================
// Handles loading resources from another entity.

import type { Entity, EntityUpdate } from '../../primitive-types/semantic/entity/entity-types.js';
import type { ActionHandler, ActionValidator, TickContext } from './actionTypes.js';

/**
 * Validates whether the load action can be performed.
 * Checks both Capability (has required systems) and State (resources available).
 */
export const loadValidate: ActionValidator = (
    _actor: Entity,
    _targets: Entity[],
    _inputs: Record<string, unknown>
): boolean => {
    // capability: does the entity have cargo capacity?
    // state: is there space in cargo and resources in source?
    // stub: always returns false until implemented
    return false;
};

/**
 * Executes the load action.
 * Rule: handler must call validate first to prevent illegal action desync.
 */
export const loadHandler: ActionHandler = (
    actor: Entity,
    targets: Entity[],
    inputs: Record<string, unknown>,
    _context: TickContext
): EntityUpdate[] => {
    // gate: validation must pass before any state mutation
    if (!loadValidate(actor, targets, inputs)) {
        return [];
    }

    // implementation goes here
    return [];
};
